...
public class LecternTransferHandler implements TransferHandler {
    ...
    public Result handle(Context context) {
        ...
        Map<BookItemVariant, Long> supply = buildSupply(lectern);
    ...
    private static boolean canSatisfy(Map<BookItemVariant, Long> supply, List<List<BookItemVariant>> slotCandidates) {
        Map<BookItemVariant, Long> remaining = new HashMap<>(supply);
    ...
    private static List<List<BookItemVariant>> gridCandidates(Display display, Map<BookItemVariant, Long> supply) {
        var slots = new ArrayList<List<BookItemVariant>>(LecternNetworking.RECIPE_GRID_SIZE);
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            var candidates = new ArrayList<BookItemVariant>();
            for (var entry : ingredient) {
                ...
                var variant = BookItemVariant.of(stack);
            ...
            if (candidates.size() > 1)
                candidates.sort(Comparator.comparingLong(
                        (BookItemVariant variant) -> supply.getOrDefault(variant, 0L)).reversed());
