...
public class LibraryNetwork {
    ...
    public static List<Entry> aggregate(List<LibraryBlockEntity> libraries) {
        ...
        for (var library : libraries) {
            for (var bookStack : library.storedBooks.getHeldStacks()) {
    ...
    public static long insertIntoLibrary(LibraryBlockEntity library, ItemStack stack) {
        var bookStacks = library.storedBooks.getHeldStacks();
        int originalCount = stack.getCount();
    ...
    public static long insert(List<LibraryBlockEntity> libraries, ItemStack stack) {
        ...
        for (var library : libraries) {
            if (stack.isEmpty())
                break;
            if (insertExisting(library.storedBooks.getHeldStacks(), stack))
                changed.add(library);
        }
        ...
        for (var library : libraries) {
            if (stack.isEmpty())
                break;
            if (insertNewTypes(library.storedBooks.getHeldStacks(), stack) && !changed.contains(library))
                changed.add(library);
        }
